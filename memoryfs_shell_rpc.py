...
if __name__ == "__main__":
  ...
  if(len(sys.argv) < 2):
    print("There is not enough arguments. Please enter NumOfServers addresses:port")
    quit()
  NumOfServers = int(sys.argv[1])
  ListOfServers = list(sys.argv)
  new_list_of_servers = []
  for i in range(NumOfServers):
    new_list_of_servers.append(ListOfServers[i+2])

  #new_list_of_servers = list([ListOfServers[2:NumOfServers+2]])
  ...
  RawBlocks = DiskBlocks(new_list_of_servers,NumOfServers)
  # Load blocks from dump file
  clean_state = False 
  RawBlocks.InitializeBlocks(clean_state,UUID)

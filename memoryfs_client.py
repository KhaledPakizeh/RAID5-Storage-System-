...
class DiskBlocks():
  def __init__(self, servers_url,NumOfServers):

    
    self.NumOfServers = NumOfServers
    self.block_server = []
    self.failed_server_type = None # serves as a flag.
    self.failed_server_number = None
    self.non_failed_server_number = None
    self.list_of_parity_location = []
    
    for i in range(len(servers_url)):
      server_url = ("http://" + str(servers_url[i]))
...
## Blocks are padded with zeroes up to BLOCK_SIZE
  ...
  def Put(self, block_number, block_data):
    ...
    if block_number in range(0,TOTAL_NUM_BLOCKS): 
      ...
      if(putdata == -1):
        print('something worng with putdata') # should never happen.

      (block_server_index,parity_server_index,physical_block_num) = self.virtual_to_physical_map(block_number)
      #figure out how to processed:
      if(block_server_index != self.failed_server_number and parity_server_index != self.failed_server_number):
        ret = self.Raid5_Put(block_number,block_data)
        if(ret == -1):
          print('one server went down call the workaround')
          ret=self.Raid_work_around(block_number,block_data)
  ...
  # used when we need to have a work around
  def update_data(self,block_number,block_data):
    (block_server_index,parity_server_index,physical_block_num) =self.virtual_to_physical_map(block_number)
    print('i am here now:')
    print('this is the server index: ',self.failed_server_number)
    print('this is the type:',self.failed_server_type)
    print('this is the other working server:',self.non_failed_server_number)
    #2. Grab realvent blocks of data from remaning servers:
    list_byte_arrays = []
    print('start correction!!')
    for i in range(self.failed_server_number):

        data = self.block_server[i].Get(physical_block_num)
        print('this is servers used :',i)
        if( data == -1):
          print('this is the problem i:',i)
          quit()
        list_byte_arrays.append(bytearray(self.block_server[i].Get(physical_block_num)))
    for j in range(self.failed_server_number+1,self.NumOfServers):
        data = self.block_server[j].Get(physical_block_num)
        if( data == -1):
          print('this is the problem j:',j)
          print('this is what we get from server j:',data)
          quit()
        list_byte_arrays.append(bytearray(self.block_server[j].Get(physical_block_num)))
      
    #2. Retreive missing data:
    correct_data = bytearray(BLOCK_SIZE)
    
    for i in range(len(list_byte_arrays)):
        correct_data = self.Xor_data(correct_data,list_byte_arrays[i])
    print('retrevied data:',correct_data)

    #3. XOR retrevied data with new data 
    
    updtated_data = self.Xor_data(correct_data,block_data)

    #4. Write the new data back to server that is still on.   
    ret = self.block_server[self.non_failed_server_number].Put(physical_block_num,updtated_data)
    
    #5. if server does not allow the data to be put, then server must be down.
    if(ret == -1):
        print('warning more than one server is down. Server number:',block_server_index)
    #6. report to top layer.
    return  ret


  def Raid_work_around(self,block_number,block_data):
      ret = self.update_data(block_number,block_data)
      return  ret
  ...
  def Raid5_Put(self,block_number,block_data):
    logging.debug ('Put: block number ' + str(block_number) + ' len ' + str(len(block_data)) + '\n' + str(block_data.hex()))
    if len(block_data) > BLOCK_SIZE:
      logging.error('Put: Block larger than BLOCK_SIZE: ' + str(len(block_data)))
      quit()
    ...
    if block_number in range(0,TOTAL_NUM_BLOCKS): 
      ...
      if(block_server_index != self.failed_server_number and parity_server_index != self.failed_server_number):
        ...
        #1. Read old data 
        try: 
          old_data = self.single_get(block_number,DATA)
          print('compare1:',old_data)
        # server failed.
        except: 
          (block_server_index,parity_server_index,physical_block_num) = self.virtual_to_physical_map(block_number)
          self.failed_server_type = Data_server
          self.failed_server_number = block_server_index
          self.non_failed_server_number = parity_server_index
          print('failed')
          quit()
        ...
        #2. Read parity block 
        try: 
          old_parity = self.single_get(block_number,PARITY)
        # server failed.
        except: 
          (block_server_index,parity_server_index,physical_block_num) = self.virtual_to_physical_map(block_number)
          self.failed_server_type = Parity_server
          self.failed_server_number = parity_server_index
          self.non_failed_server_number = block_server_index
          print('failed')
          quit()
        ...
        if(block_server_index != self.failed_server_number and parity_server_index != self.failed_server_number ):
          old_data = self.single_get(block_number,DATA)
          print('compare2:',old_data)
          old_parity = self.single_get(block_number,PARITY)
          # check sum was not correct        
          if(old_data == -1):
            print('check sum was not correct - old data')
            print('calling fix now:')
            old_data = self.Correct_lost_data(block_number)

        
          if(old_parity == -1):
            print('something worng with parity data')
            print('calling fix now:')
            old_parity = self.Correct_lost_data(block_number)

          #3. Xor the new data with old data 
          Xored_data =self.Xor_data(old_data,putdata)
          if(Xored_data == -1):
            print('something worng with Xored_data')  # should never happen 
          #4. Xor the the xored results with the parity data to get the new parity.
          new_parity_data = self.Xor_data(Xored_data,old_parity)
          if(new_parity_data == -1):
            print('something worng with new_parity_data') # should never happen 
            
          #5. Write the new data 
          ret = self.single_put(block_number,putdata,DATA)
          if ret == -1:
            logging.error('could not put new data') # should never happen 
            quit()
          return 0
          #6. Update the parity block 
          ret = self.single_put(block_number,new_parity_data,PARITY)
          if ret == -1:
            logging.error('could not put new parity') # should never happen 
            quit()
          return 0
        
        # one of the servers went down. Now we need to start working around it.
        else: 
            return -1 
...
## Equivalent to the textbook's BLOCK_NUMBER_TO_BLOCK(b)
  ...
  def Xor_data(self,byte_array1,byte_array2):
    if(byte_array1 == -1 or byte_array2 == -1):
      print('i was given a -1')
    #xOred_data = bytearray()
  #  print(type(byte_array1))
  #  print(byte_array1)
   # print('1:',len(byte_array1))
   # print('2:',len(byte_array2))
    #for i in range(len(byte_array1)):
      #xOred_data.append(byte_array1[i] ^ byte_array2[i])  
    Xored_data = bytearray(byte_array1 ^ byte_array2 for (byte_array1,byte_array2) in zip(byte_array1, byte_array2))
    return Xored_data
  ...
  def Get(self,block_number):
    ...
    data  = []
    try : 
        data = bytearray(self.single_get(block_number,DATA))
        
    except:
        data = -1
        self.failed_server_type =  Data_server # serves as a flag.
        self.failed_server_number = block_server_index
    ...
    if(data == -1):
        print('check sum failed or server is down')
        print('starting the modification process')
        data = self.Correct_lost_data(block_number)
        print('here:',data)
  ...
  def single_put(self,Vblock_number,block_data,parity_flag):
    
    (block_server_index,parity_server_index,physical_block_num) = self.virtual_to_physical_map(Vblock_number)    
    if(parity_flag == PARITY):
      ret = self.block_server[parity_server_index].Put(physical_block_num,block_data)
      self.list_of_parity_location.append(parity_server_index)
    else:
      ret = self.block_server[block_server_index].Put(physical_block_num,block_data)

    return ret 



  def single_get(self,Vblock_number,parity_flag):

    (block_server_index,parity_server_index,physical_block_num) = self.virtual_to_physical_map(Vblock_number)
    if(parity_flag== PARITY):
      ret = self.block_server[parity_server_index].Get(physical_block_num)
    else:
      ret = self.block_server[block_server_index].Get(physical_block_num)
      self.list_of_parity_location.append(parity_server_index)
    return ret

     
  def virtual_to_physical_map(self,Vblock_number):
    block_server_index = Vblock_number % (self.NumOfServers-1)
    physical_block_num =  math.floor(Vblock_number/(self.NumOfServers-1))
    parity_server_index = ((physical_block_num % self.NumOfServers)  - (self.NumOfServers-1))*-1
    if(block_server_index >= parity_server_index):
        block_server_index = (block_server_index + 1)%(self.NumOfServers)

    return(block_server_index,parity_server_index,physical_block_num) 
  


  # used when we have lost data/integerity of a block due to decay.
  def Correct_lost_data(self,Vblock_number):
    #1. check where we are at:
    (block_server_index,parity_server_index,physical_block_num) = self.virtual_to_physical_map(Vblock_number)
   # print('block_server_index, parity_server_index,physical_block_num:',block_server_index, parity_server_index,physical_block_num)
    #3. Grab remaning data:
    list_byte_arrays = []

    for i in range(block_server_index):
      list_byte_arrays.append(bytearray(self.block_server[i].Get(physical_block_num)))
    for j in range(block_server_index+1,self.NumOfServers):
    #  print('this is  j:',j)
     # print(self.block_server[j])
      list_byte_arrays.append(bytearray(self.block_server[j].Get(physical_block_num)))

   # print('this is the obtained data:',list_byte_arrays)
  #1. Xor the remaning grabbed blocks to get the correct data. 
    correct_data = bytearray(BLOCK_SIZE)
    
    for i in range(len(list_byte_arrays)):
      correct_data = self.Xor_data(correct_data,list_byte_arrays[i])
      #print('check size:',len(correct_data))
    #print('this is the correct data: ',correct_data)
    #print('this is their xor results:',correct_data)
  #2. Write the correct data back to server to replace faulty block.   (This is not the functionality disscussed in class.)
   # ret = self.block_server[block_server_index].Put(physical_block_num,correct_data)
    #print('new correct_data: ', correct_data)
  #3. if server does not allow the data to be put, then server must be down.
    #if(ret == -1):
     # print('warning server is down. Server number:',block_server_index)
  #4. Give back the upper layer the correct data
   # print(correct_data)
    return  (correct_data)
...
## Serializes and saves block[] data structure to a disk file
  ...
  def DumpToDisk(self, prefix):
    ...
    filename = str(prefix.hex()) + "_BS_" + str(BLOCK_SIZE) + "_NB_" + str(TOTAL_NUM_BLOCKS) + "_IS_" + str(INODE_SIZE) + "_MI_" + str(MAX_NUM_INODES) + ".dump"
